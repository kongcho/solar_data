...
class star(object):
    ...
    def __init__(self, kic, ffi_dir=None):
        ...
        client = kplr.API()
        self.target = client.target(kic)
        self.star = client.star(kic)
    ...
    def generate_panel(self, img):
        ...
        for i in range(4):
            g = np.where(self.qs == i)[0]
            plt.errorbar(self.times[g], self.obs_flux[g], yerr=self.flux_uncert[i], fmt=self.fmt[i])

        plt.xlabel('Time', fontsize=20)
        plt.ylabel('Relative Flux', fontsize=20)
        plt.title(self.kic, fontsize=20)
